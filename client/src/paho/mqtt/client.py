...
class MQTTAuthMessageInfo(object):
...
class Client(object):
    ...
    * Use disconnect() to disconnect from the broker
    ...
    def san_loop(self, timeout=1.0, max_packets=1):
        """Process network events.
    ...
    def san_loop_read(self, max_packets=1):
        """Process read network events. Use in place of calling loop() if you
        wish to handle your client reads as part of your own application.
    ...
    def san_loop_forever(self, timeout=1.0, max_packets=1, retry_first_connection=False):
        """This function call loop() for you in an infinite blocking loop. It
        is useful for the case where you only want to run the MQTT client loop
        in your program.
        ...
        while run:
            rc = MQTT_ERR_SUCCESS
            while rc == MQTT_ERR_SUCCESS:
                rc = self.san_loop(timeout, max_packets)
                """SANGEETH"""
                if rc == 1:
                    break
                # We don't need to worry about locking here, because we've
    ...
    def on_auth(self):
        """If implemented, called when a message that was to be sent using the
        auth() call has completed transmission to the broker."""

        return self._on_auth

    @on_auth.setter
    def on_auth(self, func):
        """Define the auth message callback implementation."""
...
#            1 + 2 + 2 + len(self._client_id)
        ...
        print("remaining length after : ", remaining_length) 
        ...
        if self._protocol == MQTTv5:
            if self._connect_properties == None:
                ...
                #sangeeth modified
                """auth_method_properties = b'\x15'
                auth_method_name = "SANGEETH"
                auth_method_data = bytearray(auth_method_name.encode("utf-8"))
                auth_method_length = unhexlify('{:04x}'.format(len(auth_method_data)))
                auth_data_properties = b'\x16'
                auth_data_data = b'\x00\x00'
                auth_data_length = unhexlify('{:04x}'.format(len(auth_data_data)))
                packed_connect_properties = b'\x00'
                
                properties_length = unhexlify('{:02x}'.format(len(auth_method_properties + auth_method_length + auth_method_data + auth_data_properties + auth_data_length + auth_data_data)))
                
                properties_payload = properties_length + auth_method_properties + auth_method_length + auth_method_data + auth_data_properties + auth_data_length + auth_data_data
                
                packed_connect_properties = properties_payload"""
        ...
        self._pack_str16(packet, date_time)#sangeeth                    
...
#Sangeeth start - NTP supported ========================================================================
    ...
    def ntp_san(self, host, port=1883, keepalive=60, bind_address="", bind_port=0,
                clean_start=MQTT_CLEAN_START_FIRST_ONLY, properties=None):
        """Connect to a remote broker.
        ...
        self.connect_async(host, port, keepalive,
                           bind_address, bind_port, clean_start, properties)
        return self.reconnect_ntp_san()

    def reconnect_ntp_san(self):
        """Reconnect the client after a disconnect. Can only be called after
        connect()/connect_async()."""
        if len(self._host) == 0:
            raise ValueError('Invalid host.')
        ...
        return self._send_connect_san(self._keepalive)

    def _send_connect_san(self, keepalive):
        proto_ver = self._protocol
        # hard-coded UTF-8 encoded string
        protocol = b"MQTT" if proto_ver >= MQTTv311 else b"MQIsdp"
...
#        remaining_length = 2 + len(protocol) + 1 + \
#            1 + 2 + 2 + len(self._client_id) + 2 + len("Fri 09 Apr 2021 01:06:04 PM EDT")
#        print("remaining length after : ", remaining_length)    
        ...
        command = CONNECT + 0x01 #NTP_mqtt request command definning
        packet = bytearray()
        packet.append(command)
...
#        remaining_length += len("date time")#sangeeth
        self._pack_remaining_length(packet, remaining_length)
        packet.extend(struct.pack("!H" + str(len(protocol)) + "sBBH", len(protocol), protocol, proto_ver, connect_flags,
                                  keepalive))
...
#        self._pack_str16(packet, "Fri 09 Apr 2021 01:06:04 PM EDT")#sangeeth                
...
#        f = self._in_packet['command'] +  self._in_packet['remaining_length'] +  
    ...
    def _handle_ntprep(self):
        ...
        if self._protocol == MQTTv5:
            ...
            if rtncode == 0:
                ...
                logger.info("[MQTT5-san-6.4] TIME NOT SYNCED between Client and Server")
                print(" ") 
        ...
        if self._protocol == MQTTv5:
            self._easy_log(
                MQTT_LOG_DEBUG, "Received NTPREP (%s, %s) properties=%s", flags, reason, properties)
        else:
            self._easy_log(
                MQTT_LOG_DEBUG, "Received NTPREP (%s, %s)", flags, result)

        # it won't be the first successful connect any more
        self._mqttv5_first_connect = False

        if result == 0:
            rc=self._send_disconnect()
            return rc
    ...
    def _handle_auth(self):
        ...
        (slen, packet) = struct.unpack(pack_format, self._in_packet['packet'])
        pack_format = '!' + str(slen) + 's' + str(len(packet) - slen) + 's'
        (packet) = struct.unpack(pack_format, packet)

        if self._protocol != MQTTv5 and len(username, password) == 0:
            return MQTT_ERR_AUTH

        if self._protocol == MQTTv5:
            message = MQTTMessage()
            message.properties = Properties(AUTH >> 4)
            props, props_len = message.properties.unpack(packet)
            packet = packet[props_len:]
        ...
        if self._protocol == MQTTv5:
           ...
           self._easy_log(
           MQTT_LOG_DEBUG,
           "Received AUTH ('%s', '%s', properties=%s, ...  (%d bytes)",
           message.username, message.password, message.properties, len(message.payload)
           )
        else:
            self._easy_log(
            MQTT_LOG_DEBUG,
            "Received AUTH ('%s', '%s'), ...  (%d bytes)",
            message.username, message.password, len(message.payload)
            )
